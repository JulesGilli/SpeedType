      ...
      <div className={`
        ...
        {showEffect && <>
            ...
            <div className={`
                ...
                animate-combo-popup
                ${effectType === 'epic' ? 'text-pink-500 text-6xl font-bold' : effectType === 'great' ? 'text-purple-500 text-5xl font-bold' : 'text-blue-400 text-4xl'}
              `}>
              {effectType === 'epic' ? 'EPIC COMBO!' : effectType === 'great' ? 'Great Combo!' : 'Good!'}
            </div>
          </>}
        {showFloatingScore && <FloatingScore score={lastScore} type={effectType} />}
        <div className={`
          ...
          <input ref={inputRef} type="text" value={userInput} onChange={handleInputChange} className="w-full bg-gray-700 text-white text-center text-2xl py-4 px-6 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 hover:bg-gray-600" autoFocus />
          <div className="mt-6 flex justify-between text-sm">
            ...
            <div>
              Combo:{' '}
              <span className={`
                  ${comboCount >= 5 ? 'text-pink-400 animate-pulse font-bold' : comboCount >= 3 ? 'text-purple-400 animate-pulse' : 'text-gray-400'}
                `}>
                {comboCount}x
              </span>
            </div>
